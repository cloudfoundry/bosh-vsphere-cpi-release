# SwaggerClient::PolicyFirewallSessionTimerProfileBindingMap
...
**sequence_number** | **Integer** | Sequence number is used to resolve conflicts when two profiles get applied to a single port. Lower value gets higher precedence. Two binding maps having the same profile path should have the same sequence number.  | [optional] 
**firewall_session_timer_profile_path** | **String** | PolicyPath of associated Firewall Timer Session Profile | 
