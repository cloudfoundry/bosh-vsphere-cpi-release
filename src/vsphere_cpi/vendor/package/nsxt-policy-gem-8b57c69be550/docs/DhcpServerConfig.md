# SwaggerClient::DhcpServerConfig
...
**server_address** | **String** | DHCP server address in CIDR format. Prefix length should be less than or equal to 30. DHCP server is deployed as DHCP relay service. IPv6 is not supported.  | 
**edge_cluster_path** | **String** | Edge cluster path. Auto assigned if only one edge cluster is configured on enforcement-point.  | [optional] 
**lease_time** | **Integer** | IP address lease time in seconds.  | [optional] [default to 86400]
