...
describe 'CPI', nsx_transformers_policy: true do
  ...
  context 'when policy api is enabled' do
  ...
  def verify_port_in_group(lports, group)
    ...
    grp_objs.each do |grp|
      exists = grp.expression.any? do |expr|
        expr.is_a?(SwaggerClient::PathExpression) && !expr.paths.nil? && !expr.paths.include?(lport_display_names)
      end
  ...
  def create_group(name:)
    grp = SwaggerClient::Group.new(id: name)
  ...
  def create_segment(name:)
    ...
    seg = SwaggerClient::Segment.new(id: name, list_transport_zones_for_enforcement_point: tz_path)
  ...
  def policy_group_api
    @policy_group_api ||= SwaggerClient::PolicyApi.new(nsxt_client)
  end

  def policy_realization_api
    @policy_realization_api ||= SwaggerClient::PolicyRealizationApi.new(nsxt_client)
  end

  def policy_segment_api
    @policy_segment_api ||= SwaggerClient::PolicyConnectivitySegmentsApi.new(nsxt_client)
  end

  def policy_port_api
    @policy_port_api ||= SwaggerClient::PolicyConnectivitySegmentsPortsApi.new(nsxt_client)
  end

  def transport_zone_api
    @transport_zone_api ||= SwaggerClient::PolicyInfraEnforcementPointsTransportZoneApi.new(nsxt_client)
  end
  ...
  def nsxt_client
    ...
    @nsxt_client = SwaggerClient::ApiClient.new(configuration)
  end
end
