  ...
  class NsxRole
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NSXT::NsxRole` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NSXT::NsxRole`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def role=(role)
      validator = EnumAttributeValidator.new('Object', ['read_only_api_users', 'read_write_api_users', 'enterprise_admin', 'auditor', 'network_engineer', 'network_op', 'security_engineer', 'security_op', 'lb_admin', 'lb_auditor', 'cloud_service_admin', 'cloud_service_auditor', 'site_reliability_engineer', 'site_reliability_auditor', 'cloud_admin', 'cloud_auditor'])
