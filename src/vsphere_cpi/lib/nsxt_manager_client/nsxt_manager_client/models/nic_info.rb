  ...
  class NicInfo
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NSXT::NicInfo` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NSXT::NicInfo`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'ip_allocation_type')
        self.ip_allocation_type = attributes[:'ip_allocation_type']
      end

      if attributes.key?(:'nic_metadata')
    ...
    def valid?
      ip_allocation_type_validator = EnumAttributeValidator.new('Object', ['STATIC', 'DHCP', 'NONE'])
    ...
    def ip_allocation_type=(ip_allocation_type)
      validator = EnumAttributeValidator.new('Object', ['STATIC', 'DHCP', 'NONE'])
