...
module NSXT
  ...
  class PerUpstreamServerStatistics
    ...
    def self.openapi_types
      {
        :'queries_failed' => :'Object',
        :'upstream_server' => :'Object',
        :'queries_succeeded' => :'Object'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NSXT::PerUpstreamServerStatistics` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NSXT::PerUpstreamServerStatistics`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'queries_failed')
        self.queries_failed = attributes[:'queries_failed']
      end

      if attributes.key?(:'upstream_server')
        self.upstream_server = attributes[:'upstream_server']
      end

      if attributes.key?(:'queries_succeeded')
