  ...
  class TransportZone
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NSXT::TransportZone` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NSXT::TransportZone`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      transport_type_validator = EnumAttributeValidator.new('', ['OVERLAY', 'VLAN'])
      return false unless transport_type_validator.valid?(@transport_type)
      host_switch_mode_validator = EnumAttributeValidator.new('', ['STANDARD', 'ENS'])
    ...
    def transport_type=(transport_type)
      validator = EnumAttributeValidator.new('', ['OVERLAY', 'VLAN'])
    ...
    def host_switch_mode=(host_switch_mode)
      validator = EnumAttributeValidator.new('', ['STANDARD', 'ENS'])
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          transport_zone_profile_ids == o.transport_zone_profile_ids && super(o)
    end
    ...
    def hash
      [_system_owned, display_name, description, tags, _create_user, _protection, _create_time, _last_modified_time, _last_modified_user, id, resource_type, is_default, transport_type, host_switch_id, host_switch_name, host_switch_mode, nested_nsx, uplink_teaming_policy_names, transport_zone_profile_ids].hash
    end
