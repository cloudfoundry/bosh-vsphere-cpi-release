  ...
  class DhcpStatistics
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NSXT::DhcpStatistics` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NSXT::DhcpStatistics`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'dhcp_server_id')
        self.dhcp_server_id = attributes[:'dhcp_server_id']
      end

      if attributes.key?(:'nacks')
      ...
      if attributes.key?(:'discovers')
        self.discovers = attributes[:'discovers']
      end

      if attributes.key?(:'acks')
      ...
      if attributes.key?(:'ip_pool_stats')
