  ...
  class HostNode
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NSXT::HostNode` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NSXT::HostNode`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'discovered_ip_addresses')
      ...
      if attributes.key?(:'discovered_node_id')
        self.discovered_node_id = attributes[:'discovered_node_id']
      end

      if attributes.key?(:'managed_by_server')
        self.managed_by_server = attributes[:'managed_by_server']
      end

      if attributes.key?(:'host_credential')
      ...
      if attributes.key?(:'maintenance_mode_state')
    ...
    def valid?
      ...
      os_type_validator = EnumAttributeValidator.new('', ['ESXI', 'RHELKVM', 'RHELSERVER', 'RHELCONTAINER', 'UBUNTUKVM', 'UBUNTUSERVER', 'HYPERV', 'CENTOSKVM', 'CENTOSSERVER', 'CENTOSCONTAINER', 'SLESKVM', 'SLESSERVER'])
      return false unless os_type_validator.valid?(@os_type)
      maintenance_mode_state_validator = EnumAttributeValidator.new('', ['OFF', 'ENTERING', 'ON'])
    ...
    def os_type=(os_type)
      validator = EnumAttributeValidator.new('', ['ESXI', 'RHELKVM', 'RHELSERVER', 'RHELCONTAINER', 'UBUNTUKVM', 'UBUNTUSERVER', 'HYPERV', 'CENTOSKVM', 'CENTOSSERVER', 'CENTOSCONTAINER', 'SLESKVM', 'SLESSERVER'])
    ...
    def maintenance_mode_state=(maintenance_mode_state)
      validator = EnumAttributeValidator.new('', ['OFF', 'ENTERING', 'ON'])
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          os_type == o.os_type &&
          maintenance_mode_state == o.maintenance_mode_state && super(o)
    end
    ...
    def hash
      [discovered_ip_addresses, ip_addresses, external_id, fqdn, resource_type, discovered_node_id, managed_by_server, host_credential, os_version, os_type, maintenance_mode_state].hash
    end
