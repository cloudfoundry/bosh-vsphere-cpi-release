  ...
  class ContainerNetworkPolicy
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NSXT::ContainerNetworkPolicy` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NSXT::ContainerNetworkPolicy`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      policy_type_validator = EnumAttributeValidator.new('', ['NETWORK_POLICY', 'ASG'])
    ...
    def policy_type=(policy_type)
      validator = EnumAttributeValidator.new('', ['NETWORK_POLICY', 'ASG'])
    ...
    def hash
      [_last_sync_time, display_name, description, resource_type, tags, container_cluster_id, policy_type, origin_properties, external_id, spec, container_project_id].hash
    end
