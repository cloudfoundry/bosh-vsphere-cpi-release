  ...
  class EdgeNode
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NSXT::EdgeNode` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NSXT::EdgeNode`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'node_settings')
        self.node_settings = attributes[:'node_settings']
      end

      if attributes.key?(:'deployment_config')
        self.deployment_config = attributes[:'deployment_config']
      end

      if attributes.key?(:'allocation_list')
    ...
    def hash
      [discovered_ip_addresses, ip_addresses, external_id, fqdn, resource_type, node_settings, deployment_config, allocation_list, deployment_type].hash
    end
