  ...
  class UpgradeUnitGroupAggregateInfo
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NSXT::UpgradeUnitGroupAggregateInfo` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NSXT::UpgradeUnitGroupAggregateInfo`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'upgrade_unit_count')
      ...
      if attributes.key?(:'post_upgrade_status')
      ...
      if attributes.key?(:'upgrade_units')
        if (value = attributes[:'upgrade_units']).is_a?(Array)
          self.upgrade_units = value
        end
      end

      if attributes.key?(:'extended_configuration')
    ...
    def hash
      [_system_owned, display_name, description, tags, _create_user, _protection, _create_time, _last_modified_time, _last_modified_user, id, resource_type, status, upgrade_unit_count, failed_count, type, percent_complete, post_upgrade_status, enabled, upgrade_units, extended_configuration, parallel].hash
    end
