...
module NSXT
  ...
  class ServiceDeploymentSpec
    ...
    def self.openapi_types
      {
        :'deployment_specs' => :'Object',
        :'nic_metadata_list' => :'Object',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NSXT::ServiceDeploymentSpec` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NSXT::ServiceDeploymentSpec`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'deployment_specs')
        if (value = attributes[:'deployment_specs']).is_a?(Array)
          self.deployment_specs = value
        end
      end

      if attributes.key?(:'nic_metadata_list')
        if (value = attributes[:'nic_metadata_list']).is_a?(Array)
          self.nic_metadata_list = value
        end
      end

      if attributes.key?(:'deployment_template')
        if (value = attributes[:'deployment_template']).is_a?(Array)
          self.deployment_template = value
        end
      end

      if attributes.key?(:'svm_version')
