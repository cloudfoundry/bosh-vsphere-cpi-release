  ...
  class LogicalRouterUpLinkPort
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NSXT::LogicalRouterUpLinkPort` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NSXT::LogicalRouterUpLinkPort`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'service_bindings')
    ...
    def valid?
      ...
      resource_type_validator = EnumAttributeValidator.new('', ['LogicalRouterUpLinkPort', 'LogicalRouterDownLinkPort', 'LogicalRouterLinkPortOnTIER0', 'LogicalRouterLinkPortOnTIER1', 'LogicalRouterLoopbackPort', 'LogicalRouterIPTunnelPort', 'LogicalRouterCentralizedServicePort'])
      return false unless resource_type_validator.valid?(@resource_type)
      return false if @subnets.nil?
      return false if @edge_cluster_member_index.nil?
      urpf_mode_validator = EnumAttributeValidator.new('', ['NONE', 'STRICT'])
    ...
    def resource_type=(resource_type)
      validator = EnumAttributeValidator.new('', ['LogicalRouterUpLinkPort', 'LogicalRouterDownLinkPort', 'LogicalRouterLinkPortOnTIER0', 'LogicalRouterLinkPortOnTIER1', 'LogicalRouterLoopbackPort', 'LogicalRouterIPTunnelPort', 'LogicalRouterCentralizedServicePort'])
    ...
    def urpf_mode=(urpf_mode)
      validator = EnumAttributeValidator.new('', ['NONE', 'STRICT'])
    ...
    def hash
      [logical_router_id, service_bindings, resource_type, subnets, linked_logical_switch_port_id, edge_cluster_member_index, urpf_mode, mac_address, ndra_prefix_config, ndra_profile_id, mtu].hash
    end
