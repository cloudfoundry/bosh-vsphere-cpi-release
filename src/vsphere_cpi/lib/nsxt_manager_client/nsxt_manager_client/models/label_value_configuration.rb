  ...
  class LabelValueConfiguration
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NSXT::LabelValueConfiguration` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NSXT::LabelValueConfiguration`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      sub_type_validator = EnumAttributeValidator.new('', ['VERTICALLY_ALIGNED'])
    ...
    def sub_type=(sub_type)
      validator = EnumAttributeValidator.new('', ['VERTICALLY_ALIGNED'])
    ...
    def hash
      [resource_type, display_name, datasources, weight, icons, shared, footer, drilldown_id, is_drilldown, legend, layout, navigation, sub_type, properties].hash
    end
