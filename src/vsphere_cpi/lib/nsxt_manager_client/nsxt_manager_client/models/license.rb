  ...
  class License
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NSXT::License` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NSXT::License`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'is_eval')
        self.is_eval = attributes[:'is_eval']
      end

      if attributes.key?(:'is_mh')
    ...
    def valid?
      capacity_type_validator = EnumAttributeValidator.new('', ['VM', 'CPU', 'USER'])
    ...
    def capacity_type=(capacity_type)
      validator = EnumAttributeValidator.new('', ['VM', 'CPU', 'USER'])
