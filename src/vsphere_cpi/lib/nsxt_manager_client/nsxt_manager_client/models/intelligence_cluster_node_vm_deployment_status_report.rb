  ...
  class IntelligenceClusterNodeVMDeploymentStatusReport
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NSXT::IntelligenceClusterNodeVMDeploymentStatusReport` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NSXT::IntelligenceClusterNodeVMDeploymentStatusReport`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('Object', ['NOT_AVAILABLE', 'VM_DEPLOYMENT_QUEUED', 'VM_DEPLOYMENT_IN_PROGRESS', 'VM_DEPLOYMENT_FAILED', 'VM_POWER_ON_IN_PROGRESS', 'VM_POWER_ON_FAILED', 'VM_POWER_ON_SUCCESSFUL', 'VM_REGISTRATION_FAILED', 'VM_WAITING_TO_COME_ONLINE', 'VM_ONLINE_FAILED', 'VM_POWER_OFF_IN_PROGRESS', 'VM_POWER_OFF_FAILED', 'WAITING_TO_UNDEPLOY_VM', 'VM_UNDEPLOY_IN_PROGRESS', 'VM_UNDEPLOY_FAILED', 'VM_UNDEPLOY_SUCCESSFUL', 'UNKNOWN_STATE'])
