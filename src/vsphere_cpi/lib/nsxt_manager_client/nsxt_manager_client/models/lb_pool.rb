  ...
  class LbPool
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NSXT::LbPool` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NSXT::LbPool`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'member_group')
        self.member_group = attributes[:'member_group']
      end

      if attributes.key?(:'snat_translation')
    ...
    def valid?
      algorithm_validator = EnumAttributeValidator.new('', ['ROUND_ROBIN', 'WEIGHTED_ROUND_ROBIN', 'LEAST_CONNECTION', 'WEIGHTED_LEAST_CONNECTION', 'IP_HASH'])
    ...
    def algorithm=(algorithm)
      validator = EnumAttributeValidator.new('', ['ROUND_ROBIN', 'WEIGHTED_ROUND_ROBIN', 'LEAST_CONNECTION', 'WEIGHTED_LEAST_CONNECTION', 'IP_HASH'])
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          min_active_members == o.min_active_members && super(o)
    end
    ...
    def hash
      [_system_owned, display_name, description, tags, _create_user, _protection, _create_time, _last_modified_time, _last_modified_user, id, resource_type, member_group, snat_translation, algorithm, members, passive_monitor_id, tcp_multiplexing_number, active_monitor_ids, tcp_multiplexing_enabled, min_active_members].hash
    end
