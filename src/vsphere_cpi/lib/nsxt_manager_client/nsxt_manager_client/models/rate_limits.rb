  ...
  class RateLimits
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NSXT::RateLimits` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NSXT::RateLimits`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'rx_multicast')
        self.rx_multicast = attributes[:'rx_multicast']
      else
        self.rx_multicast = 0
      end

      if attributes.key?(:'tx_multicast')
      ...
      if attributes.key?(:'tx_broadcast')
        self.tx_broadcast = attributes[:'tx_broadcast']
      else
        self.tx_broadcast = 0
      end

      if attributes.key?(:'rx_broadcast')
