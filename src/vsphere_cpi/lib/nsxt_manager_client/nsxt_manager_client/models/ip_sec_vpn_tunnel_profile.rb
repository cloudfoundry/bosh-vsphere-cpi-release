  ...
  class IPSecVPNTunnelProfile
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NSXT::IPSecVPNTunnelProfile` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NSXT::IPSecVPNTunnelProfile`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'df_policy')
        self.df_policy = attributes[:'df_policy']
      else
        self.df_policy = 'COPY'
      end

      if attributes.key?(:'sa_life_time')
    ...
    def valid?
      encapsulation_mode_validator = EnumAttributeValidator.new('', ['TUNNEL_MODE'])
      return false unless encapsulation_mode_validator.valid?(@encapsulation_mode)
      transform_protocol_validator = EnumAttributeValidator.new('', ['ESP'])
      return false unless transform_protocol_validator.valid?(@transform_protocol)
      df_policy_validator = EnumAttributeValidator.new('', ['COPY', 'CLEAR'])
    ...
    def encapsulation_mode=(encapsulation_mode)
      validator = EnumAttributeValidator.new('', ['TUNNEL_MODE'])
    ...
    def transform_protocol=(transform_protocol)
      validator = EnumAttributeValidator.new('', ['ESP'])
    ...
    def df_policy=(df_policy)
      validator = EnumAttributeValidator.new('', ['COPY', 'CLEAR'])
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          sa_life_time == o.sa_life_time && super(o)
    end
    ...
    def hash
      [_system_owned, display_name, description, tags, _create_user, _protection, _create_time, _last_modified_time, _last_modified_user, id, resource_type, encapsulation_mode, transform_protocol, digest_algorithms, encryption_algorithms, enable_perfect_forward_secrecy, dh_groups, df_policy, sa_life_time].hash
    end
