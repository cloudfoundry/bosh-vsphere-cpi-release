  ...
  class VirtualMachine
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NSXT::VirtualMachine` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NSXT::VirtualMachine`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'local_id_on_host')
    ...
    def valid?
      ...
      type_validator = EnumAttributeValidator.new('', ['EDGE', 'SERVICE', 'REGULAR'])
      return false unless type_validator.valid?(@type)
      return false if @power_state.nil?
      power_state_validator = EnumAttributeValidator.new('', ['VM_RUNNING', 'VM_STOPPED', 'VM_SUSPENDED', 'UNKNOWN'])
    ...
    def power_state=(power_state)
      validator = EnumAttributeValidator.new('', ['VM_RUNNING', 'VM_STOPPED', 'VM_SUSPENDED', 'UNKNOWN'])
    ...
    def hash
      [_last_sync_time, display_name, description, resource_type, tags, source, local_id_on_host, type, guest_info, power_state, compute_ids, host_id, external_id].hash
    end
