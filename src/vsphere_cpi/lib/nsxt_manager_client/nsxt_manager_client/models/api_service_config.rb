...
module NSXT
  ...
  class ApiServiceConfig
    ...
    def self.openapi_types
      {
        :'global_api_concurrency_limit' => :'Object',
        :'lockout_immune_addresses' => :'Object',
        :'client_api_concurrency_limit' => :'Object',
        :'session_timeout' => :'Object',
        :'client_api_rate_limit' => :'Object',
        :'redirect_host' => :'Object',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NSXT::ApiServiceConfig` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NSXT::ApiServiceConfig`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'global_api_concurrency_limit')
        self.global_api_concurrency_limit = attributes[:'global_api_concurrency_limit']
      else
        self.global_api_concurrency_limit = 199
      end

      if attributes.key?(:'lockout_immune_addresses')
        if (value = attributes[:'lockout_immune_addresses']).is_a?(Array)
          self.lockout_immune_addresses = value
        end
      end

      if attributes.key?(:'client_api_concurrency_limit')
      ...
      if attributes.key?(:'client_api_rate_limit')
      ...
      if attributes.key?(:'protocol_versions')
