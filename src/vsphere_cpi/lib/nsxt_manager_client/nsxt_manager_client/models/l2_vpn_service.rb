  ...
  class L2VpnService
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NSXT::L2VpnService` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NSXT::L2VpnService`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'logical_tap_ip_pool')
    ...
    def valid?
      ...
      mode_validator = EnumAttributeValidator.new('', ['SERVER', 'CLIENT'])
    ...
    def mode=(mode)
      validator = EnumAttributeValidator.new('', ['SERVER', 'CLIENT'])
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          mode == o.mode &&
          logical_tap_ip_pool == o.logical_tap_ip_pool && super(o)
    end
    ...
    def hash
      [_system_owned, display_name, description, tags, _create_user, _protection, _create_time, _last_modified_time, _last_modified_user, id, resource_type, enable_full_mesh, enable_hub, logical_router_id, mode, logical_tap_ip_pool].hash
    end
