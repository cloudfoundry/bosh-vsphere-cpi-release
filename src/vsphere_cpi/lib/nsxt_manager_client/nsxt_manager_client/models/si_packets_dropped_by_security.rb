...
module NSXT
  class SIPacketsDroppedBySecurity
    ...
    def self.openapi_types
      {
        :'spoof_guard_dropped' => :'Object',
        :'dhcp_server_dropped_ipv4' => :'Object',
        :'dhcp_server_dropped_ipv6' => :'Object',
        :'dhcp_client_dropped_ipv4' => :'Object',
        :'bpdu_filter_dropped' => :'Object',
        :'dhcp_client_dropped_ipv6' => :'Object'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NSXT::SIPacketsDroppedBySecurity` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NSXT::SIPacketsDroppedBySecurity`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'spoof_guard_dropped')
        if (value = attributes[:'spoof_guard_dropped']).is_a?(Array)
          self.spoof_guard_dropped = value
        end
      end

      if attributes.key?(:'dhcp_server_dropped_ipv4')
        self.dhcp_server_dropped_ipv4 = attributes[:'dhcp_server_dropped_ipv4']
      end

      if attributes.key?(:'dhcp_server_dropped_ipv6')
        self.dhcp_server_dropped_ipv6 = attributes[:'dhcp_server_dropped_ipv6']
      end

      if attributes.key?(:'dhcp_client_dropped_ipv4')
        self.dhcp_client_dropped_ipv4 = attributes[:'dhcp_client_dropped_ipv4']
      end

      if attributes.key?(:'bpdu_filter_dropped')
        self.bpdu_filter_dropped = attributes[:'bpdu_filter_dropped']
      end

      if attributes.key?(:'dhcp_client_dropped_ipv6')
