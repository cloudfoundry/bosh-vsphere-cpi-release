  ...
  class IPSecVPNPeerEndpoint
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NSXT::IPSecVPNPeerEndpoint` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NSXT::IPSecVPNPeerEndpoint`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'ipsec_tunnel_profile_id')
    ...
    def valid?
      ...
      authentication_mode_validator = EnumAttributeValidator.new('', ['PSK', 'CERTIFICATE'])
      return false unless authentication_mode_validator.valid?(@authentication_mode)
      return false if @peer_address.nil?
      connection_initiation_mode_validator = EnumAttributeValidator.new('', ['INITIATOR', 'RESPOND_ONLY', 'ON_DEMAND'])
    ...
    def authentication_mode=(authentication_mode)
      validator = EnumAttributeValidator.new('', ['PSK', 'CERTIFICATE'])
    ...
    def connection_initiation_mode=(connection_initiation_mode)
      validator = EnumAttributeValidator.new('', ['INITIATOR', 'RESPOND_ONLY', 'ON_DEMAND'])
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          ike_profile_id == o.ike_profile_id && super(o)
    end
    ...
    def hash
      [_system_owned, display_name, description, tags, _create_user, _protection, _create_time, _last_modified_time, _last_modified_user, id, resource_type, psk, peer_id, ipsec_tunnel_profile_id, authentication_mode, peer_address, connection_initiation_mode, dpd_profile_id, ike_profile_id].hash
    end
