  ...
  class BridgeEndpointProfile
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NSXT::BridgeEndpointProfile` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NSXT::BridgeEndpointProfile`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      high_availability_mode_validator = EnumAttributeValidator.new('', ['ACTIVE_STANDBY'])
    ...
    def high_availability_mode=(high_availability_mode)
      validator = EnumAttributeValidator.new('', ['ACTIVE_STANDBY'])
    ...
    def hash
      [_system_owned, display_name, description, tags, _create_user, _protection, _create_time, _last_modified_time, _last_modified_user, id, resource_type, failover_mode, edge_cluster_member_indexes, high_availability_mode, edge_cluster_id].hash
    end
