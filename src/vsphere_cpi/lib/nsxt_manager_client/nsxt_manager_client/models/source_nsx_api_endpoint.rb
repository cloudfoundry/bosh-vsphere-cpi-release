  ...
  class SourceNsxApiEndpoint
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NSXT::SourceNsxApiEndpoint` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NSXT::SourceNsxApiEndpoint`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'vc_port')
        self.vc_port = attributes[:'vc_port']
      else
        self.vc_port = 443
      end

      if attributes.key?(:'vc_username')
        self.vc_username = attributes[:'vc_username']
      end

      if attributes.key?(:'vc_ip')
