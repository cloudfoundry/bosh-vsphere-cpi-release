  ...
  class NodeStatusProperties
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NSXT::NodeStatusProperties` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NSXT::NodeStatusProperties`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'non_dpdk_cpu_cores')
        self.non_dpdk_cpu_cores = attributes[:'non_dpdk_cpu_cores']
      end

      if attributes.key?(:'swap_total')
      ...
      if attributes.key?(:'dpdk_cpu_cores')
        self.dpdk_cpu_cores = attributes[:'dpdk_cpu_cores']
      end

      if attributes.key?(:'file_systems')
    ...
    def valid?
      source_validator = EnumAttributeValidator.new('', ['realtime', 'cached'])
    ...
    def source=(source)
      validator = EnumAttributeValidator.new('', ['realtime', 'cached'])
