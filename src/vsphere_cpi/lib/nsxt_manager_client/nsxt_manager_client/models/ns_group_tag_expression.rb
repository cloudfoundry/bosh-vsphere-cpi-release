  ...
  class NSGroupTagExpression
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NSXT::NSGroupTagExpression` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NSXT::NSGroupTagExpression`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      tag_op_validator = EnumAttributeValidator.new('', ['EQUALS', 'CONTAINS', 'STARTSWITH', 'ENDSWITH'])
      return false unless tag_op_validator.valid?(@tag_op)
      scope_op_validator = EnumAttributeValidator.new('', ['EQUALS'])
      return false unless scope_op_validator.valid?(@scope_op)
      return false if @target_type.nil?
      target_type_validator = EnumAttributeValidator.new('', ['LogicalSwitch', 'LogicalPort', 'VirtualMachine', 'IPSet'])
    ...
    def tag_op=(tag_op)
      validator = EnumAttributeValidator.new('', ['EQUALS', 'CONTAINS', 'STARTSWITH', 'ENDSWITH'])
    ...
    def scope_op=(scope_op)
      validator = EnumAttributeValidator.new('', ['EQUALS'])
    ...
    def target_type=(target_type)
      validator = EnumAttributeValidator.new('', ['LogicalSwitch', 'LogicalPort', 'VirtualMachine', 'IPSet'])
