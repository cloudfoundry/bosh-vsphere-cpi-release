...
module Nsxt9PolicyClient
  class LoadBalancerSSLCiphersAndProtocolsApi
    ...
    def list_ssl_ciphers_and_protocols_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: LoadBalancerSSLCiphersAndProtocolsApi.list_ssl_ciphers_and_protocols ...'
      end
      if @api_client.config.client_side_validation && !opts[:'page_size'].nil? && opts[:'page_size'] > 1000
        fail ArgumentError, 'invalid value for "opts[:"page_size"]" when calling LoadBalancerSSLCiphersAndProtocolsApi.list_ssl_ciphers_and_protocols, must be smaller than or equal to 1000.'
      end

      if @api_client.config.client_side_validation && !opts[:'page_size'].nil? && opts[:'page_size'] < 0
        fail ArgumentError, 'invalid value for "opts[:"page_size"]" when calling LoadBalancerSSLCiphersAndProtocolsApi.list_ssl_ciphers_and_protocols, must be greater than or equal to 0.'
      end
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: LoadBalancerSSLCiphersAndProtocolsApi#list_ssl_ciphers_and_protocols\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
