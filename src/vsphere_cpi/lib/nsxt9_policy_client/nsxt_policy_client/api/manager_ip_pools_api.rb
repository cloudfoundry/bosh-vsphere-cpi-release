  ...
  class ManagerIPPoolsApi
    ...
    def list_manager_ip_pools_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: ManagerIPPoolsApi.list_manager_ip_pools ...'
      end
      if @api_client.config.client_side_validation && !opts[:'page_size'].nil? && opts[:'page_size'] > 1000
        fail ArgumentError, 'invalid value for "opts[:"page_size"]" when calling ManagerIPPoolsApi.list_manager_ip_pools, must be smaller than or equal to 1000.'
      end

      if @api_client.config.client_side_validation && !opts[:'page_size'].nil? && opts[:'page_size'] < 0
        fail ArgumentError, 'invalid value for "opts[:"page_size"]" when calling ManagerIPPoolsApi.list_manager_ip_pools, must be greater than or equal to 0.'
      end
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: ManagerIPPoolsApi#list_manager_ip_pools\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
    ...
    def read_manager_ip_pool_with_http_info(manager_ip_pool_id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: ManagerIPPoolsApi.read_manager_ip_pool ...'
      end
      ...
      if @api_client.config.client_side_validation && manager_ip_pool_id.nil?
        fail ArgumentError, "Missing the required parameter 'manager_ip_pool_id' when calling ManagerIPPoolsApi.read_manager_ip_pool"
      end
      # resource path
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: ManagerIPPoolsApi#read_manager_ip_pool\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
  end
