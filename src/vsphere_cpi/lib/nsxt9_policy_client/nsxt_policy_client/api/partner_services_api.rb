  ...
  class PartnerServicesApi
    ...
    def read_partner_service_with_http_info(service_name, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: PartnerServicesApi.read_partner_service ...'
      end
      # verify the required parameter 'service_name' is set
      if @api_client.config.client_side_validation && service_name.nil?
        fail ArgumentError, "Missing the required parameter 'service_name' when calling PartnerServicesApi.read_partner_service"
      end
      # resource path
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: PartnerServicesApi#read_partner_service\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
    ...
    def read_partner_services_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: PartnerServicesApi.read_partner_services ...'
      end
      if @api_client.config.client_side_validation && !opts[:'page_size'].nil? && opts[:'page_size'] > 1000
        fail ArgumentError, 'invalid value for "opts[:"page_size"]" when calling PartnerServicesApi.read_partner_services, must be smaller than or equal to 1000.'
      end

      if @api_client.config.client_side_validation && !opts[:'page_size'].nil? && opts[:'page_size'] < 0
        fail ArgumentError, 'invalid value for "opts[:"page_size"]" when calling PartnerServicesApi.read_partner_services, must be greater than or equal to 0.'
      end
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: PartnerServicesApi#read_partner_services\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
  end
