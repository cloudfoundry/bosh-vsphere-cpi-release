  ...
  class FinetuningApi
    ...
    def list_resource_info_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: FinetuningApi.list_resource_info ...'
      end
      if @api_client.config.client_side_validation && !opts[:'page_size'].nil? && opts[:'page_size'] > 1000
        fail ArgumentError, 'invalid value for "opts[:"page_size"]" when calling FinetuningApi.list_resource_info, must be smaller than or equal to 1000.'
      end

      if @api_client.config.client_side_validation && !opts[:'page_size'].nil? && opts[:'page_size'] < 0
        fail ArgumentError, 'invalid value for "opts[:"page_size"]" when calling FinetuningApi.list_resource_info, must be greater than or equal to 0.'
      end
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: FinetuningApi#list_resource_info\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
