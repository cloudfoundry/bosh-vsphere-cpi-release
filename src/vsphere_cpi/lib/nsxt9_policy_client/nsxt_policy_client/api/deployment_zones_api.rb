  ...
  class DeploymentZonesApi
    ...
    def list_deployment_zones_for_infra_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: DeploymentZonesApi.list_deployment_zones_for_infra ...'
      end
      if @api_client.config.client_side_validation && !opts[:'page_size'].nil? && opts[:'page_size'] > 1000
        fail ArgumentError, 'invalid value for "opts[:"page_size"]" when calling DeploymentZonesApi.list_deployment_zones_for_infra, must be smaller than or equal to 1000.'
      end

      if @api_client.config.client_side_validation && !opts[:'page_size'].nil? && opts[:'page_size'] < 0
        fail ArgumentError, 'invalid value for "opts[:"page_size"]" when calling DeploymentZonesApi.list_deployment_zones_for_infra, must be greater than or equal to 0.'
      end
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: DeploymentZonesApi#list_deployment_zones_for_infra\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
    ...
    def read_deployment_zone_infra_with_http_info(deployment_zone_id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: DeploymentZonesApi.read_deployment_zone_infra ...'
      end
      ...
      if @api_client.config.client_side_validation && deployment_zone_id.nil?
        fail ArgumentError, "Missing the required parameter 'deployment_zone_id' when calling DeploymentZonesApi.read_deployment_zone_infra"
      end
      # resource path
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: DeploymentZonesApi#read_deployment_zone_infra\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
  end
