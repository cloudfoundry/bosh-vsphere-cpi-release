...
module Nsxt9PolicyClient
  # Policy Edge Transport is the object,where edge transport node related parameters are configured. 
  class PolicyEdgeTransportNode
    attr_accessor :_self
    ...
    # Set failure domain of edge transport node which will help in auto placement of TIER1 logical routers, DHCP Servers and MDProxies, if failure domain based allocation is enabled in edge cluster API. In case failure domain is not set by user explicitly, it will be always assigned with default system created failure domain. 
    attr_accessor :failure_domain_path

    # The edge node deployment type may be physical or virtual. 
    attr_accessor :deployment_type

    # This field contains the info of type of edge transport node. 
    ...
    def self.swagger_types
      {
        ...
        :'form_factor' => :'String',
        :'hostname' => :'String',
        :'vm_deployment_config' => :'PolicyVmDeploymentConfig',
        :'appliance_config' => :'PolicyVmApplianceConfig',
        :'credentials' => :'PolicyEdgeTransportNodeCredential',
        :'management_interface' => :'PolicyEdgeTransportManagementInterface',
        :'switch_spec' => :'PolicyEdgeTransportNodeSwitchSpec',
        :'advanced_configuration' => :'Array<KeyValuePair>'
      }
    end
    ...
    def initialize(attributes = {})
      ...
      if attributes.has_key?(:'failure_domain_path')
        self.failure_domain_path = attributes[:'failure_domain_path']
      end
      ...
      if attributes.has_key?(:'appliance_config')
        self.appliance_config = attributes[:'appliance_config']
      end
      ...
      if attributes.has_key?(:'management_interface')
        self.management_interface = attributes[:'management_interface']
      end

      if attributes.has_key?(:'switch_spec')
        self.switch_spec = attributes[:'switch_spec']
      end

      if attributes.has_key?(:'advanced_configuration')
        if (value = attributes[:'advanced_configuration']).is_a?(Array)
          self.advanced_configuration = value
        end
      end
    end
    ...
    def list_invalid_properties
      ...
      if @hostname !~ Regexp.new(/^(?=.{1,255}$)[0-9A-Za-z](?:(?:[0-9A-Za-z]|-){0,61}[0-9A-Za-z])?(?:\\.[0-9A-Za-z](?:(?:[0-9A-Za-z]|-){0,61}[0-9A-Za-z])?)*?$/)
    ...
    def valid?
      ...
      node_type_validator = EnumAttributeValidator.new('String', ['EDGE_NODE'])
      return false unless node_type_validator.valid?(@node_type)
      form_factor_validator = EnumAttributeValidator.new('String', ['SMALL', 'MEDIUM', 'LARGE', 'XLARGE'])
      return false unless form_factor_validator.valid?(@form_factor)
      return false if @hostname.nil?
      return false if @hostname !~ Regexp.new(/^(?=.{1,255}$)[0-9A-Za-z](?:(?:[0-9A-Za-z]|-){0,61}[0-9A-Za-z])?(?:\\.[0-9A-Za-z](?:(?:[0-9A-Za-z]|-){0,61}[0-9A-Za-z])?)*?$/)
      true
    end
    ...
    def node_type=(node_type)
      validator = EnumAttributeValidator.new('String', ['EDGE_NODE'])
    ...
    def form_factor=(form_factor)
      validator = EnumAttributeValidator.new('String', ['SMALL', 'MEDIUM', 'LARGE', 'XLARGE'])
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          failure_domain_path == o.failure_domain_path &&
          deployment_type == o.deployment_type &&
          node_type == o.node_type &&
          form_factor == o.form_factor &&
          hostname == o.hostname &&
          vm_deployment_config == o.vm_deployment_config &&
          appliance_config == o.appliance_config &&
          credentials == o.credentials &&
          management_interface == o.management_interface &&
          switch_spec == o.switch_spec &&
          advanced_configuration == o.advanced_configuration
    end
    ...
    def hash
      [_self, _links, _schema, _revision, resource_type, _create_user, _create_time, _last_modified_user, _last_modified_time, _system_owned, _protection, id, display_name, description, tags, path, relative_path, parent_path, remote_path, unique_id, realization_id, owner_id, origin_site_id, children, marked_for_delete, overridden, failure_domain_path, deployment_type, node_type, form_factor, hostname, vm_deployment_config, appliance_config, credentials, management_interface, switch_spec, advanced_configuration].hash
    end
