...
module Nsxt9PolicyClient
  ...
  class LBVirtualServer
    ...
    # Path to optional object that enables persistence on a virtual server allowing related client connections to be sent to the same backend server. Persistence is deactivated by default. 
    ...
    def hash
      [_self, _links, _schema, _revision, resource_type, _create_user, _create_time, _last_modified_user, _last_modified_time, _system_owned, _protection, id, display_name, description, tags, path, relative_path, parent_path, remote_path, unique_id, realization_id, owner_id, origin_site_id, children, marked_for_delete, overridden, enabled, ip_address, ports, access_log_enabled, lb_persistence_profile_path, lb_service_path, pool_path, application_profile_path, client_ssl_profile_binding, server_ssl_profile_binding, rules, default_pool_member_ports, sorry_pool_path, max_concurrent_connections, max_new_connection_rate, access_list_control, log_significant_event_only].hash
    end
