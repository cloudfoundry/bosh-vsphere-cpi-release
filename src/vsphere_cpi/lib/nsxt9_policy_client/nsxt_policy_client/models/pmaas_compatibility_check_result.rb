...
module Nsxt9PolicyClient
  # Result of prechecks run for onboarding remote site. The checks include NSX version compatibility with NSX+, Round Trip Time (RTT), etc. Note that some of checks like RTT are soft limits.  While the NSX+ compatible field remains PENDING the client should continue to intermittently re-read to determine when the validation is complete. 
  class PmaasCompatibilityCheckResult
    attr_accessor :_self
    ...
    # Result of prechecks run for onboarding remote site. While the NSX+ compatible field remains PENDING the client should continue to intermittently re-read to determine when the validation is complete. 
    attr_accessor :pmaas_compatible

    # Result of prechecks run for onboarding remoet site to NSX+. When the Site is found to be INCOMPATIBLE this field will contain appropriate information or error message. 
    attr_accessor :compatibility_error_message
    ...
    def self.attribute_map
      {
        ...
        :'pmaas_compatible' => :'pmaas_compatible',
        :'compatibility_error_message' => :'compatibility_error_message'
      }
    end
    ...
    def initialize(attributes = {})
      ...
      if attributes.has_key?(:'pmaas_compatible')
        self.pmaas_compatible = attributes[:'pmaas_compatible']
      else
        self.pmaas_compatible = 'PENDING'
      end

      if attributes.has_key?(:'compatibility_error_message')
        self.compatibility_error_message = attributes[:'compatibility_error_message']
      end
    end
    ...
    def valid?
      ...
      pmaas_compatible_validator = EnumAttributeValidator.new('String', ['COMPATIBLE', 'PENDING', 'INCOMPATIBLE'])
      return false unless pmaas_compatible_validator.valid?(@pmaas_compatible)
      true
    end
    ...
    # @param [Object] pmaas_compatible Object to be assigned
    def pmaas_compatible=(pmaas_compatible)
      validator = EnumAttributeValidator.new('String', ['COMPATIBLE', 'PENDING', 'INCOMPATIBLE'])
      unless validator.valid?(pmaas_compatible)
        fail ArgumentError, 'invalid value for "pmaas_compatible", must be one of #{validator.allowable_values}.'
      end
      @pmaas_compatible = pmaas_compatible
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          pmaas_compatible == o.pmaas_compatible &&
          compatibility_error_message == o.compatibility_error_message
    end
