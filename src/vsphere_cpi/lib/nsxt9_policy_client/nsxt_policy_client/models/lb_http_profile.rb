...
module Nsxt9PolicyClient
  ...
  class LBHttpProfile
    ...
    # When buffering is deactivated, the response is passed to a client synchronously, immediately as it is received. When buffering is activated, LB receives a response from the backend server as soon as possible, saving it into the buffers. 
    ...
    def hash
      [_self, _links, _schema, _revision, resource_type, _create_user, _create_time, _last_modified_user, _last_modified_time, _system_owned, _protection, id, display_name, description, tags, path, relative_path, parent_path, remote_path, unique_id, realization_id, owner_id, origin_site_id, children, marked_for_delete, overridden, x_forwarded_for, http_redirect_to, http_redirect_to_https, ntlm, idle_timeout, request_header_size, request_body_size, response_timeout, response_header_size, response_buffering, server_keep_alive].hash
    end
