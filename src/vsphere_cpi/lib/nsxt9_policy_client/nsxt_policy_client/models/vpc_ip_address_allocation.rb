...
module Nsxt9PolicyClient
  # Allocation parameters for the IP address (e.g. specific IP address) can be specified. VPC IP allocation object is used for associating IP/CIDR to VPC manually. External VPC allocation IP will be used in NAT and subnet port's manual external address binding. 
  class VpcIpAddressAllocation
    attr_accessor :_self
    ...
    # This defines the type of IP address block that will be used to allocate IP. This field is applicable only if IP addressType at VPC is DUAL. In case of IPv4, external blocks will be used, and in case of IPv6, IPv6 blocks will be used. 
    attr_accessor :ip_address_type

    # Represents visibility of IP address block. This field is not applicable if IPAddressType at VPC is IPv6. 
    attr_accessor :ip_address_block_visibility

    # Single IP Address that is allocated from external ip block or IPv6 block based on IP address type. If not specified, any available IP will be allocated from respective IP block. If specified, it has to be within range of respective IP blocks. If IP is already in use then validation error will be thrown. This field will be replaced with allocation_ips. 
    attr_accessor :allocation_ip

    # IP block path for subnet IP allocation. 
    attr_accessor :ip_block

    # The customer needs to pass allocation_ips or allocation_size and ip_address_block_visibility. If allocation_size is used, the system will allocate IP addresses from unused IP addresses. Range or comma separated ip string is not supported in VPC IP allocation. 
    attr_accessor :allocation_ips
    ...
    def initialize(attributes = {})
      ...
      if attributes.has_key?(:'ip_address_block_visibility')
        self.ip_address_block_visibility = attributes[:'ip_address_block_visibility']
      else
        self.ip_address_block_visibility = 'EXTERNAL'
      end
      ...
      if attributes.has_key?(:'ip_block')
        self.ip_block = attributes[:'ip_block']
      end

      if attributes.has_key?(:'allocation_ips')
        self.allocation_ips = attributes[:'allocation_ips']
      end
    ...
    def valid?
      ...
      ip_address_block_visibility_validator = EnumAttributeValidator.new('String', ['EXTERNAL', 'PRIVATE', 'PRIVATE_TGW'])
      return false unless ip_address_block_visibility_validator.valid?(@ip_address_block_visibility)
      true
    end
    ...
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] ip_address_block_visibility Object to be assigned
    def ip_address_block_visibility=(ip_address_block_visibility)
      validator = EnumAttributeValidator.new('String', ['EXTERNAL', 'PRIVATE', 'PRIVATE_TGW'])
      unless validator.valid?(ip_address_block_visibility)
        fail ArgumentError, 'invalid value for "ip_address_block_visibility", must be one of #{validator.allowable_values}.'
      end
      @ip_address_block_visibility = ip_address_block_visibility
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          ip_address_block_visibility == o.ip_address_block_visibility &&
          allocation_ip == o.allocation_ip &&
          ip_block == o.ip_block &&
          allocation_ips == o.allocation_ips &&
