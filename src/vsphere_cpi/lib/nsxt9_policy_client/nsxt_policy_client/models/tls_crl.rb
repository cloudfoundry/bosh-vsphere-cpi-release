...
module Nsxt9PolicyClient
  class TlsCrl
    ...
    attr_accessor :one_crl

    # Certificates revoked by issuer and serial number
    attr_accessor :details_revoked_by_issuer_and_serial_number

    # Certificates revoked by subject and public key hash
    attr_accessor :details_revoked_by_subject_and_public_key_hash
    ...
    def self.attribute_map
      {
        ...
        :'details' => :'details',
        :'one_crl' => :'one_crl',
        :'details_revoked_by_issuer_and_serial_number' => :'details_revoked_by_issuer_and_serial_number',
        :'details_revoked_by_subject_and_public_key_hash' => :'details_revoked_by_subject_and_public_key_hash'
      }
    end
    ...
    def self.swagger_types
      {
        ...
        :'details' => :'X509Crl',
        :'one_crl' => :'String',
        :'details_revoked_by_issuer_and_serial_number' => :'Array<IssuerSerialNumber>',
        :'details_revoked_by_subject_and_public_key_hash' => :'Array<SubjectPublicKeyHash>'
      }
    end
    ...
    def initialize(attributes = {})
      ...
      if attributes.has_key?(:'crl_type')
        self.crl_type = attributes[:'crl_type']
      else
        self.crl_type = 'X509'
      end
      ...
      if attributes.has_key?(:'one_crl')
        self.one_crl = attributes[:'one_crl']
      end

      if attributes.has_key?(:'details_revoked_by_issuer_and_serial_number')
        if (value = attributes[:'details_revoked_by_issuer_and_serial_number']).is_a?(Array)
          self.details_revoked_by_issuer_and_serial_number = value
        end
      end

      if attributes.has_key?(:'details_revoked_by_subject_and_public_key_hash')
        if (value = attributes[:'details_revoked_by_subject_and_public_key_hash']).is_a?(Array)
          self.details_revoked_by_subject_and_public_key_hash = value
        end
      end
    end
    ...
    def valid?
      ...
      crl_type_validator = EnumAttributeValidator.new('String', ['OneCRL', 'X509'])
      return false unless crl_type_validator.valid?(@crl_type)
      true
    end
    ...
    # @param [Object] crl_type Object to be assigned
    def crl_type=(crl_type)
      validator = EnumAttributeValidator.new('String', ['OneCRL', 'X509'])
      unless validator.valid?(crl_type)
        fail ArgumentError, 'invalid value for "crl_type", must be one of #{validator.allowable_values}.'
      end
      @crl_type = crl_type
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          details == o.details &&
          one_crl == o.one_crl &&
          details_revoked_by_issuer_and_serial_number == o.details_revoked_by_issuer_and_serial_number &&
          details_revoked_by_subject_and_public_key_hash == o.details_revoked_by_subject_and_public_key_hash
    end
    ...
    def hash
      [_self, _links, _schema, _revision, resource_type, _create_user, _create_time, _last_modified_user, _last_modified_time, _system_owned, _protection, id, display_name, description, tags, path, relative_path, parent_path, remote_path, unique_id, realization_id, owner_id, origin_site_id, children, marked_for_delete, overridden, crl_type, pem_encoded, details, one_crl, details_revoked_by_issuer_and_serial_number, details_revoked_by_subject_and_public_key_hash].hash
    end
