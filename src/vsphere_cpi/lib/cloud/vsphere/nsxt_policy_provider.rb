...
module VSphereCloud
  ...
  class NSXTPolicyProvider
    ...
    def self.build_policy_api_client(config, logger)
      ...
      NSXTPolicyClient::ApiClient.new(configuration)
    end
    ...
    def create_segment_port(segment:, tags:)
      ...
      port_tags = tags.each_pair.map do |type, value|
        NSXTPolicyClient::Tag.new(scope: "cpi/#{type}", tag: value)
      end

      port_attachment = NSXTPolicyClient::PortAttachment.new(id: "#{SecureRandom.uuid}")#, type: DEFAULT_VIF_TYPE)
      segment_port = NSXTPolicyClient::SegmentPort.new(id: port_id, display_name: port_id,
    ...
    def remove_vm_from_groups(vm)
      ...
      all_groups.each do |grp|
        retry_on_conflict("while removing vm: #{vm.cid} from group #{grp.id}") do
          ...
          modified_grp = false
          all_path_expressions = grp.expression.select{|expr| expr.is_a?(NSXTPolicyClient::PathExpression)}
      ...
      segment_port_list.each do |seg_name, port_id|
        ...
        begin
          retry_on_conflict("while adding tag: #{metadata['id']} to segment: #{seg_name} and port: #{port_id} for vm: #{vm.cid}") do
            ...
            id_tag = NSXTPolicyClient::Tag.new(scope: 'bosh/id', tag: Digest::SHA1.hexdigest(metadata['id']))
    ...
    DEFAULT_SLEEP = 1
    MAX_TRIES=100
    DEFAULT_VIF_TYPE = 'INDEPENDENT'.freeze
    ...
    def get_or_create_path_expression(group_obj)
      ...
      if group_obj.expression.empty?
        group_obj.expression << NSXTPolicyClient::PathExpression.new(resource_type: 'PathExpression',
      ...
      path_expression = group_obj.expression.detect{|expr| expr.is_a?(NSXTPolicyClient::PathExpression)}
      ...
      group_obj.expression << NSXTPolicyClient::ConjunctionOperator.new(resource_type: 'ConjunctionOperator',
    ...
    def policy_group_api
      @policy_group_api ||= NSXTPolicyClient::PolicyApi.new(@client)
    end

    def policy_realization_api
      @policy_realization_api ||= NSXTPolicyClient::PolicyRealizationApi.new(@client)
    end

    def policy_segment_port_api
      @policy_segment_port_api ||= NSXTPolicyClient::PolicyConnectivitySegmentsPortsApi.new(@client)
    end

    def policy_segment_api
      @policy_segment_api ||= NSXTPolicyClient::PolicyConnectivitySegmentsApi.new(@client)
    end
    ...
    def retry_on_conflict(log_str)
      ...
      Bosh::Retryable.new(
        ...
        begin
          yield
        rescue NSXTPolicyClient::ApiCallError => e
  ...
  module RevisionConflictMatcher
    ...
    def matches?(exception)
      exception.is_a?(NSXTPolicyClient::ApiCallError) && [409, 412].include?(exception.code)
    end
