...
module VSphereCloud
  class VmCreator
    ...
    def create(vm_config)
      created_vm = nil
      times = 0
      # Loop to retry on certain failures
      loop do
        vm_config.validate
        ...
        if times == 0
          ...
          logger.info("Retrying vm: #{vm_config.name} creation : #{times} time ")
        end
        times+=1
        ...
        if vm_config.attach_gpu?
          ...
          host_resource = cluster_placement[cluster.name][:host_array]&.shuffle.first
        ...
        begin
          ...
          # If a pci device is already in use , retry vm creation
          unless e.is_a?(VSphereCloud::VCenterClient::GenericVmConfigPciInUse)
            raise e
          else
