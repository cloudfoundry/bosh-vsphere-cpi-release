    ...
    def create(vm_config)
      ...
      loop do
        ...
        vm_config.cluster_placements.each do |cluster_placement|
          ...
          if tries == 0
            logger.info("Trying vm: #{vm_config.name} creation for first time.")
          else
            logger.info("Retrying vm: #{vm_config.name} creation : #{tries} times ")
          end
          tries+=1
          ...
          logger.info("Cloning VM on Datastore: #{datastore.name}, Cluster: #{cluster.name}, Host: #{host_resource.nil? ? " Chosen by DRS" : host_resource.name}")
          ...
          begin
            ...
            if e.is_a?(VSphereCloud::VCenterClient::GenericVmConfigPciInUse)
              logger.info("Failed to create vm #{vm_config.name} because one of the PCI device is already in use. Retrying....")
            else
              raise e
            end
          else
            #Breaking out of retry logic (Successful VM Creation)
            return created_vm
          end
        end
      end
