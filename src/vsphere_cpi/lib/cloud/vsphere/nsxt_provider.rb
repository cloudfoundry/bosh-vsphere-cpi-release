    ...
    def on_delete_vm(vm_id)
      ...
      nsgroups = @client.nsgroups.select do |nsgroup|
        nsgroup.members.find do |member|
          member.target_type == 'LogicalPort' && member.target_property == 'id' && member.value == logical_port.id
        end
      end

      nsgroups.each { |nsgroup| nsgroup.remove_member(logical_port) }
    end
  end
end