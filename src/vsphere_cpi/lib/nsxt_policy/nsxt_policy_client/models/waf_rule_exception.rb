...
module NSXTPolicyClient
  # Rule exceptions are used to allow Security Admin to modify a rule to eliminate false positives. Similar to global exceptions, Security Admin can specify conditions like matching on the client IP address or the URI being accessed to selectively modify a rule. 
  class WAFRuleException
    # If request matches any WAFException in the list, the request will not be validated by this rule. 
