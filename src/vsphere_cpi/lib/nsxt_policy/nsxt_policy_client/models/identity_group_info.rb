...
module NSXTPolicyClient
  ...
  class IdentityGroupInfo
    # Each LDAP object is uniquely identified by its distinguished name (DN). A DN is a sequence of relative distinguished names (RDN) connected by commas. e.g. CN=Larry Cole,CN=admin,DC=corp,DC=acme,DC=com. A fully distinguished name should be provided to include specific groups else groups matching this name will be included. Example - if \"distinguished_name\": \"group50\" is provided then in that case all the groups containing \"group50\" in their name will be considered. If we specify the fully distingushed name like \"distinguished_name\": \"CN=group50, OU=default,DC=idfw,DC=local\" then only \"group50\" will be considered(if such group exists). 
