  ...
  class ApiClient
    ...
    def convert_to_type(data, return_type)
      ...
      when /\AHash\<String, (.+)\>\z/
        ...
        # models, e.g. Pet
        NSXTPolicyClient.const_get(return_type).new.tap do |model|
    ...
    def build_request_url(path)
      # This is needed because URI.encode does not handle all characters as well as CGI.escape
      # Whole URl should not be encoded. Instead individual components should be encoded.
      # Doing this way I lose the advantage of having a '/' in the name as that will not be encoded.
      path=path.split('/').map do |path_comp|
      CGI.escape(path_comp)
      end.join('/')
      # Add leading and trailing slashes to path
      path = "/#{path}".gsub(/\/+/, '/')
      # Encode base URl and add path
