...
module NSXT
  ...
  class NSGroupTagExpression
    ...
    def list_invalid_properties
      ...
      if !@scope.nil? && @scope.to_s.length > 30
        invalid_properties.push("invalid value for 'scope', the character length must be smaller than or equal to 30.")
      end

      if !@tag.nil? && @tag.to_s.length > 40
        invalid_properties.push("invalid value for 'tag', the character length must be smaller than or equal to 40.")
      end
    ...
    def valid?
      ...
      tag_op_validator = EnumAttributeValidator.new('String', ["EQUALS"])
      return false unless tag_op_validator.valid?(@tag_op)
      return false if !@scope.nil? && @scope.to_s.length > 30
      scope_op_validator = EnumAttributeValidator.new('String', ["EQUALS"])
      return false unless scope_op_validator.valid?(@scope_op)
      return false if !@tag.nil? && @tag.to_s.length > 40
      return false if @target_type.nil?
      target_type_validator = EnumAttributeValidator.new('String', ["LogicalSwitch", "LogicalPort", "VirtualMachine"])
    ...
    def scope=(scope)
      ...
      if !scope.nil? && scope.to_s.length > 30
        fail ArgumentError, "invalid value for 'scope', the character length must be smaller than or equal to 30."
      end
    ...
    def tag=(tag)
      ...
      if !tag.nil? && tag.to_s.length > 40
        fail ArgumentError, "invalid value for 'tag', the character length must be smaller than or equal to 40."
      end
    ...
    def target_type=(target_type)
      validator = EnumAttributeValidator.new('String', ["LogicalSwitch", "LogicalPort", "VirtualMachine"])
