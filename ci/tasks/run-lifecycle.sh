...
if [ "$BOSH_VSPHERE_CPI_NSXT_HOST" != "null" ]; then
  ...
  BOSH_NSXT_CERT_HOST_NAME="$(openssl x509 -noout -text -in nsxt-manager-cert.pem | awk '/X509v3 Subject Alternative Name/ {getline;gsub(/ /, "", $0); print}' | sed -E 's/DNS:|IPAddress://g' | sed 's/^\*\./nsx-mgr./')"
